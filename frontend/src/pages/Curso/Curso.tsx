import { useEffect, useState } from 'react';
import { useParams, Link } from 'react-router-dom';
import { api } from '../../services/api';
import { Header } from '../../components/Header/Header';
import styles from './Curso.module.css';

interface Anexo {
  id: number;
  nomeArquivo: string;
}

interface CursoDetalhado {
  id: number;
  titulo: string;
  texto: string;
  linkAuxiliar?: string;
  inseridoEm: string;
  anexos: Anexo[];
}

export function Curso() {
  const { id } = useParams();
  const [curso, setCurso] = useState<CursoDetalhado | null>(null);
  const [imagemSrc, setImagemSrc] = useState<string>("");
  const [loading, setLoading] = useState(true);

	useEffect(() => {
	  async function carregarDados() {
		try {
		  const response = await api.get(`/materias/${id}`);   // CORRIGIDO
		  const dadosCurso = response.data;
		  setCurso(dadosCurso);

			if (dadosCurso.anexos && dadosCurso.anexos.length > 0) {
			  const anexos = dadosCurso.anexos;
			  const ultimoAnexo = anexos[anexos.length - 1]; // pega o último anexo
			  const anexoId = ultimoAnexo.id;

			  try {
				const responseImg = await api.get(`/anexos/${anexoId}`);
				if (responseImg.data.base64) {
				  setImagemSrc(responseImg.data.base64);
				}
			  } catch (e) {
				console.warn('Falha ao carregar imagem do anexo', anexoId);
			  }
			}
		} catch (error) {
		  console.error(error);
		} finally {
		  setLoading(false);
		}
	  }

	  carregarDados();
	}, [id]);



  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50">
        <Header />
        <div className={styles.container} style={{ textAlign: 'center', marginTop: '50px' }}>
          <p style={{ color: '#fd6925', fontWeight: 'bold' }}>Carregando...</p>
        </div>
      </div>
    );
  }

  if (!curso) {
    return (
      <div className="min-h-screen bg-gray-50">
        <Header />
        <div className={styles.container}>
          <h1>Curso não encontrado.</h1>
          <Link to="/" className={styles.backLink}>Voltar</Link>
        </div>
      </div>
    );
  }

  return (
    <div className="bg-gray-50 min-h-screen">
      <Header />

      <main className={styles.container}>
        
        <Link to="/" className={styles.backLink}>
          &larr; Voltar para o Início
        </Link>

        <article className={styles.article}>
          
          {imagemSrc ? (
             <img 
               src={imagemSrc} 
               alt={curso.titulo} 
               className={styles.heroImage}
             />
          ) : (
             <img 
               src="https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?auto=format&fit=crop&w=1200&q=80" 
               alt="Placeholder" 
               className={styles.heroImage}
             />
          )}

          <div className={styles.content}>
            <div className={styles.meta}>
              <span className={styles.date}>
                {new Date(curso.inseridoEm).toLocaleDateString('pt-BR', {
                  day: 'numeric',
                  month: 'long',
                  year: 'numeric'
                })}
              </span>
            </div>

            <h1 className={styles.title}>{curso.titulo}</h1>

            <div className={styles.textBody}>
              {curso.texto}
            </div>

            {curso.linkAuxiliar && (
              <div style={{ marginTop: '30px', borderTop: '1px solid #eee', paddingTop: '20px' }}>
                <span style={{ fontWeight: 'bold', color: '#666' }}>Referência: </span>
                <a 
                  href={curso.linkAuxiliar} 
                  target="_blank" 
                  rel="noopener noreferrer"
                  style={{ color: '#2563eb', textDecoration: 'underline' }}
                >
                  {curso.linkAuxiliar}
                </a>
              </div>
            )}
          </div>
        </article>

      </main>
    </div>
  );
}